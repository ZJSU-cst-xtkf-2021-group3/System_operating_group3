			...
			<view @click="onclickInput" :class="[vertical?'tm-input-col':'flex-between ']" :style="{
				...
				<view v-if="leftIcon||title" class="tm-input-left flex-start flex-shrink" :class="[vertical?'pb-24':'']">
					...
					<view v-if="title" class="d-inline-block  "
						:class="[`text-size-${titleFontSize}`,titleClass,black_tmeme?'bk  text-grey-lighten-3':'']">
				...
				<view  class="flex-between " :class="[disabled?'opacity-6':'',]" style="width: 100%;">
					...
					<view class="tm-input-center relative fulled" >
						<view class="flex-start  px-16 tm-input-center-wk"
							:class="['round-'+round,
							...
							<view class="flex-shrink pr-12 flex-center" v-if="prefixpIcon" style="line-height: 0;">
								<tm-icons dense  :name="prefixpIcon" :size="28" :color="color_tmeme" ></tm-icons>
							</view>
							<view class="flex-shrink pr-24 text-size-n" :class="[titleClass,black_tmeme?'bk  text-grey-lighten-3':'']" v-if="prefixpText">
								{{prefixpText}}
							</view>
							
							
							<input v-if="inputType!='textarea'"  @confirm="$emit('confirm',$event)" @input="input"
								...
								:class="['text-size-n','text-align-'+align,showError?'text-red':'',' py-5 ']" :placeholder="placeholder"
							...
							<textarea v-if="inputType=='textarea'"  @confirm="$emit('confirm',$event)" @input="input"
								@keyboardheightchange="$emit('keyboardheightchange',$event)" @blur="blur"
								@focus="focusFun" :focus="focus_fs" :maxlength="maxlength" :adjust-position="adjustPosition"
								:auto-focus="autoFocus"  :confirm-type="confirmType" :disabled="disabled"
								...
								:class="[maxlength>0?'pb-46':'','text-size-n','text-align-'+align,showError?'text-red':'','pt-10 fulled']" :placeholder="placeholder"
								:placeholder-class="black_tmeme? 'text-grey-darken-1 ':' text-grey-lighten-1 ' +` text-size-n `+ placeholderClass" >	
							</textarea>
							<!-- 清除图标 -->
							...
							<view v-if="suffixIcon" class="flex-center">
								<tm-icons :size="26" :name="suffixIcon" :color="color_tmeme"></tm-icons>
							</view>
						</view>
					...
					<view class="tm-input-right flex-end flex-shrink">
						<!-- 后缀文字，比如单位，等 -->
						<text v-if="suffix" class="text-size-n text-grey-darken-4 pl-10">{{suffix}}</text>
						<!-- 后台图标。 -->
