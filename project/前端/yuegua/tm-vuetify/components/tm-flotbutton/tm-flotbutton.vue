	...
	<view class="nobody fulled"
		...
		<view
			...
			<view class="flotbtnId" :style="pos" :class="[absolute?'absolute':'',fixed?'fixed':'']">
				...
				<slot name="default">
					<tm-button  @click="click" :showValue="showText" vertical :label="label" :fontSize="fontSize"
			...
			posfun(){
				...
				if (!this.absolute && this.fixed) {
					...
					if (this.position == 'topLeft') {
						...
						let safbo = 0
						// #ifdef H5
						if(sy.safeAreaInsets.bottom===0){
							safbo = 0;
						}else{
							safbo = sy.screenHeight - sy.windowHeight - Math.abs(sy.safeAreaInsets.bottom)
						}
						// #endif
						// #ifndef H5
						safbo = sy.screenHeight - sy.windowHeight - Math.abs(sy.safeAreaInsets.bottom)
						// #endif
						safbo = Math.abs(safbo)
						...
						let safbo = 0
						// #ifdef H5
						if(js.safeAreaInsets.bottom===0){
							safbo = 0;
						}else{
							safbo = js.screenHeight - js.windowHeight - Math.abs(js.safeAreaInsets.bottom)
						}
						// #endif
						// #ifndef H5
						safbo = js.screenHeight - js.windowHeight - Math.abs(js.safeAreaInsets.bottom)
