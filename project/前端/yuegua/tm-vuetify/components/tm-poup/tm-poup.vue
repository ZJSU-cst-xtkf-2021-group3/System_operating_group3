	...
	<view class="tm-poups "  @click.stop="">
		...
		<block v-if="position_sv != 'center'">
			<view v-show="value==true&&position_sv != 'center'" class="tm-poup " :class="[
				...
				<scroll-view @click.stop.prevent="" class="tm-poup-wk" scroll-y="true" :class="[
		...
		<view v-if="value===true&&position_sv == 'center'" class="tm-poup " :class="[
			...
			<!-- 内容 -->
			<scroll-view @click.stop.prevent="" class="tm-poup-wk " scroll-y="true" :class="[
					`round-${round}`,aniOn ? 'on okck' : 'off', position_sv,
					black_tmeme ? 'grey-darken-5 bk' : bgColor
				]" :style="{
					width: width_w,
					height: height_h
	...
	.tm-poup {
		...
		.tm-poup-wk {
			...
			&.bottom {
				...
				// top: 0;
				&.on {
					animation: bottomTtop 0.30s linear;
					transform: translateY(0%);
				}

				&.off {
					animation: bottomTtop_off 0.25s ease-in-out;
					transform: translateY(100%);
				}
			}
			...
			&.top {
				...
				&.on {
					animation: topTbottom 0.3s linear;
					transform: translateY(0%);
				}

				&.off {
					animation: topTbottom_off 0.25s linear;
					transform: translateY(-100%);
				}
			}
			...
			&.left {
				...
				&.on {
					animation: leftTright 0.3s linear;

					transform: translateX(0%);
				}

				&.off {
					animation: leftTright_off 0.25s linear;
					transform: translateX(-100%);
				}
			}
			...
			&.right {
				...
				&.on {
					animation: rightTleft 0.3s linear;
					transform: translateX(0);
				}

				&.off {
					animation: rightTleft_off 0.25s linear;
					transform: translateX(100%);
				}
			}
			...
			&.center {
				opacity:0;
				&.on {
					animation: Centerleft 0.3s linear;
					opacity: 1;
				}

				&.off {
					animation: Centerleft_off 0.25s linear;
					opacity: 0;
