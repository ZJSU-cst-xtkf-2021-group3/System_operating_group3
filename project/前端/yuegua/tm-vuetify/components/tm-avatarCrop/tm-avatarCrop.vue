<template>
	<view class="tm-avatarCrop absolute t-0 l-0 black" :style="{ width: `${width}px`, height: `${height}px` }">
			...
			>
				...
				<block v-for="(item, index) in 4" :key="index">
					<view
						...
						:class="{
							'tm-avatarCrop-area-top-left': index == 0,
							'tm-avatarCrop-area-top-right': index == 1,
							'tm-avatarCrop-area-bottom-left': index == 2,
							'tm-avatarCrop-area-bottom-right': index == 3
						}"
						:id="`${index}`"
					>
	...
	methods: {
		async jishunTopData() {
			this.$nextTick(async function() {
				this.listData = [];
				let p = await this.$Querey('.tm-avatarCrop-pos', this, 50, true).catch(e => {});
				let list = p[0];
				let dr = [...this.posArray];
					...
					uni.canvasToTempFilePath(
						{
							...
							y: 0,
							width: t.area_width,
							height: t.area_height,
							canvasId: 'AvatarCrop',
							success: function(res) {
								// 在H5平台下，tempFilePath 为 base64
								uni.hideLoading();
								t.$nextTick(function() {
									t.$emit('confirm', { width: t.area_width, height: t.area_height, src: res.tempFilePath });
									t.confirm({ width: t.area_width, height: t.area_height, src: res.tempFilePath });
								});
							},
							fail: function(res) {
								uni.$tm.toast('请重试');
