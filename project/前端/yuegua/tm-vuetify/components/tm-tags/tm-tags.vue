	...
	size == 'xs' ? 'tm-tags-xs' : '',aniOpen?'showClose':''
		...
		>
			<view v-if="showClose" :class="[color_tmeme,black_tmeme?'bk':'',aniOn?'aniOn':'']" class="tm-tags-close   flex-center border-white-a-2 rounded ">
				<view style="transform: scale(0.7);"><tm-icons color="white" dense @click="closeTags" name="icon-times" :size="22"></tm-icons></view>
			</view>
			<slot></slot>
		</view>
	</view>
...
@keyframes doudong {
	0%{
		transform: rotate(6deg) translateX(4rpx) translateY(4rpx);
	}
	25%{
